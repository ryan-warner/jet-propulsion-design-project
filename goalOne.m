...
turbojetGroundSolver = @(fuelAirRatio) turbojetGroundRoll.engineCalc(fuelAirRatio, 0.1, 0, 30, 0.01, 1).specificThrust - 2800;
turbojetGroundResult = fsolve(turbojetGroundSolver, 0.1)

turbojetCruiseSolver = @(fuelAirRatio) turbojetCruise.engineCalc(fuelAirRatio, 0.1, 0.86, 30, 0.01, 1).specificThrust - 860;
turbojetCruiseResult = fsolve(turbojetCruiseSolver, 0.1)

disp("Ground Roll Optimization - Turbojet")
goalOneOpt(turbojetGroundRoll, 0, 2800, [turbojetGroundResult, 0.1, 0, 30, 0.01, 1, 2800])
disp("Cruise Optimization - Turbojet")
goalOneOpt(turbojetCruise, 0.86, 860, [turbojetCruiseResult, 0.1, 0.86, 30, 0.01, 1, 860])
